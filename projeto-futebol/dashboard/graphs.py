...
def graph_cards_per_team(data=cards_data):
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=10, b=50)  # Define as margens do gráfico
    )
...
def graph_results_per_team(time="Internacional"):
    ...
    fig.update_layout(
        ...
        paper_bgcolor='rgba(44, 146, 213, 0.06)',
        xaxis_title='',
        yaxis_title='Quantidade de Vitórias',
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=-0.25,
...
def goals(data=goals_data):
    # Contar o número de gols marcados por cada equipe
    gols_equipe = data['clube'].value_counts().reset_index()
    gols_equipe.columns = ['Equipe', 'Gols']

    fig = px.scatter(gols_equipe,x="Equipe", y="Gols", color='Gols')
